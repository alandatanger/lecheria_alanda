...
export function GlovoSection() {
  ...
  const t = commonTranslations.glovo[language]
        ...
        <div className="flex flex-col md:flex-row items-center gap-12 rounded-3xl bg-white dark:bg-slate-900 shadow-2xl overflow-hidden border border-emerald-100 dark:border-emerald-900/30">
          <div className="w-full md:w-1/2 relative h-64 md:h-96 group overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-6">
              <div className="bg-emerald-500 text-white text-xs font-bold px-3 py-1 rounded-full animate-bounce">{t.badge}</div>
            </div>
          </div>
          ...
          <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col items-start text-left">
            <AnimatedSection animation="fade-up">
              <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-4">
                {t.title.split(" ").slice(0, -2).join(" ")} <span className="text-emerald-500">{t.title.split(" ").slice(-2).join(" ")}</span>
              </h2>
              <p className="text-slate-600 dark:text-gray-300 text-lg mb-8 leading-relaxed">{t.description}</p>
              ...
              <p className="mt-4 text-xs text-slate-500 dark:text-gray-500 italic">{t.note}</p>
            </AnimatedSection>
          </div>
        </div>
      </div>
