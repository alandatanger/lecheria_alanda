...
export function MenuSidebar({ categories, activeCategory, onSelectCategory }: MenuSidebarProps) {
  ...
  return (
    <aside className="w-64 bg-slate-50 dark:bg-slate-950 border-r border-slate-200 dark:border-slate-900 h-full min-h-screen hidden lg:block sticky top-20 overflow-y-auto custom-scrollbar transition-colors duration-300">
      <div className="p-6">
        ...
        <nav className="space-y-1 stagger-children">
          {categories.map((cat, index) => (
            <AnimatedSection key={cat.id} animation="fade-left" delay={index * 50} className="w-full">
              <button
                ...
                className={cn(
                  ...
                  activeCategory === cat.id
                    ...
                    : "text-slate-600 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:bg-white dark:hover:bg-slate-900 hover:border-slate-200 dark:hover:border-slate-800",
                )}
              >
                <div
                  className={cn(
                    "p-2 rounded-lg transition-colors shadow-inner",
                    activeCategory === cat.id ? "bg-black/20 text-amber-100" : "bg-white dark:bg-slate-900 group-hover:bg-white dark:group-hover:bg-slate-800",
                  )}
                >
                  <Coffee size={18} />
                </div>
