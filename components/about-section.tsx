...
export function AboutSection() {
  ...
  const t = commonTranslations.about[language]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <AnimatedSection animation="fade-right" className="flex flex-col items-center lg:items-start text-center lg:text-left">
            <span className="font-handwriting text-3xl md:text-4xl text-amber-500 mb-2 transform -rotate-2">{t.eyebrow}</span>
            <h2 className="text-4xl md:text-6xl font-bold text-slate-900 dark:text-white mb-6 tracking-tight uppercase">ALANDA</h2>
            ...
            <div className="mt-10">
              <div className="inline-block p-4 border border-amber-500/30 rounded-full">
                <span className="font-serif italic text-slate-500 dark:text-gray-400">{t.quote}</span>
              </div>
            </div>
          </AnimatedSection>
          ...
          <AnimatedSection animation="fade-left" delay={200} className="relative group">
            ...
            <div className="absolute -bottom-6 -left-6 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xl border border-amber-500/20 max-w-[200px] hidden md:block transform hover:scale-105 transition-transform">
              ...
              <p className="text-sm font-bold text-slate-700 dark:text-gray-300 uppercase tracking-wider">{t.badge}</p>
            </div>
          </AnimatedSection>
        </div>
      </div>
